...
function signup(){
  ...
  alert('Compte créé avec succès, bonus 1500 FCFA ajouté'); showDashboardSection('home'); document.getElementById('balance').innerText=balance;
}
function login(){
...
function generateRechargeCode(){ 
  ...
  if(amount<2000){ alert('Dépôt minimum 2000 FCFA'); return; }
...
function confirmRecharge(){ 
  let amount=parseInt(document.getElementById('recharge-amount').value);
  balance+=amount; document.getElementById('balance').innerText=balance; alert('Paiement validé !');
}
function withdrawFromMenu(){ 
  ...
  balance-=amount; document.getElementById('balance').innerText=balance; alert('Retrait effectué ! (Taxes 15% appliquées)');
}